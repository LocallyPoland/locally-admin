...
const Layout = ({logout, order: {orders}, getOrders}) => {
    ...
    return (
        <div className={s.main__container}>
            ...
            <ModalWindow isVisible={isModalVisible} id={orderId} setVisibility={setModalVisible}/>
            <div className={s.orders__container}>
                <div className={s.orders__container__wrapper}>
                    ...
                    <div className={s.orders__column}>
                        {
                            orders.activeOrders && orders.activeOrders.map(active => {
                                return <button key={active._id} onClick={()=>showModal(active._id)} className={s.order__button}>
                                    <ActiveOrders
                            ...
                            orders.ordersHistory && orders.ordersHistory.map(history => {
                                return <button key={history._id} onClick={()=>showModal(history._id)} className={s.order__button}>
                                    <ActiveOrders
                                        ...
                                        orderWeight={history.weight || "5"}
                                        orderTime={history.deliveryTime || "12.20"}
                                        orderStartPlace={history.pickUp || `${history.deliveryStreet}, ${history.deliveryHouse}`}
                                        orderFinishPlace={`${history.deliveryStreet}, ${history.deliveryHouse}`}
